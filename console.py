...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, args):
        ...
        if class_name not in HBNBCommand.classes:
            ...
            return
        match = re.findall(r"(\w+=\s?.+)", " ".join(args[1:]))
        if match:
    ...
    def __parse_nums(self, string):
        ...
        if string.isdigit() or (string.startswith("-") and "." not in string):
            return int(string)
        elif string.replace(".", "").isnumeric() or string.startswith("-"):
            return float(string)
        return string
    ...
    def __create_dict(self, args):
        new_dict = {}
        for match in args:
            k, v = map(self.__parse_nums, match.split(":"))
            new_dict[k] = v.replace("_", " ") if isinstance(v, str) else v
        return new_dict
